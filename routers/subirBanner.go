...
/*SubirBanner sube la imagen al servidor */
func SubirBanner(w http.ResponseWriter, r *http.Request) {

	file, handler, _ := r.FormFile("banner")                             // Procesa como si fuera un formulario de HTML y el nombre del archivo avatar
	var extension = strings.Split(handler.Filename, ".")[1]              // Devuelve un String [1] porque sino devolveria un verctor, hace un split del .
	var archivo string = "uploads/banners" + IDUsuario + "." + extension // La ruta donde va a guardar el archivo, le agrega el id del usuario y la extensión del archivo
